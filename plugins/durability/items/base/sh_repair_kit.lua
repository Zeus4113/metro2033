...
if (SERVER) then
	...
	function ITEM:UseRepair(item, client)
		...
		local attribute = character:GetAttribute("gunsmithing", 0)
		local durability = item:GetData("durability", maxDurability)
		local amount = self.durability + (attribute / 5)

		item:SetData("durability", math.Clamp(durability + amount, 0, maxDurability))
	end
end
