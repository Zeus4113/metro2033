...
function PLUGIN:PopulateRecipeTooltip(tooltip, recipe)
	...
	if (recipe.skills) then
		...
		for v, k in pairs(recipe.skills) do
			if k > 0 then index = index + 1 end
		end

		if index == table.Count(recipe.skills) then
			local skills = tooltip:AddRow("skills")
			skills:SetText(L("SKILLS"))
			skills:SetBackgroundColor(Color(154, 199, 118))
			...
			for v, k in pairs(recipe.skills) do
				...
				skillString = skillString .. v .. " " .. k .. ", "
			end

			if (skillString != "") then
				local skills = tooltip:AddRow("skillList")
				skills:SetText("- "..string.sub(skillString, 0, #skillString-2))
				skills:SizeToContents()
			end
		end
	end
