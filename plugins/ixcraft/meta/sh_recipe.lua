...
function RECIPE:OnCanSee(client)
    ...
    if(self.skills)then    
        ...
        if index == table.Count(self.skills) then
            hasSkills = true
        end
    else
        hasSkills = true
...
function RECIPE:OnCanCraft(client)
	...
	local bHasItems, bHasTools, bHasSkill
	local missing = ""
	...
	if(bHasSkill == false) then
		return false, "You lack the required skills to craft this."
	end
			...
			for _, itemTable in pairs(inventory:GetItems()) do
				...
				if (self.requirements[uniqueID]) then
					...
					if (amountRemoved < amount) then
						...
						if itemTable.slot and itemTable.Unequip and itemTable:GetData("equip", false) then 
							itemTable:Unequip(client)
						end

						if itemTable.equipSlot and character:GetEquipment()[itemTable.equipSlot] == itemTable:GetID() then 
							character:SetEquipmentSlot(itemTable.equipSlot, nil)
						end
